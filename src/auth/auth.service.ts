...
/**
 * Owns every authentication flow. Kept separate from UsersService on
 * purpose: UsersService is a plain data-access layer for the User
 * collection, while AuthService is business logic (hashing, tokens,
 * lockout policy, orchestrating the mail service). Mixing the two would
 * make UsersService — which other modules may reasonably depend on for
 * simple lookups — implicitly carry authentication concerns.
 */
@Injectable()
